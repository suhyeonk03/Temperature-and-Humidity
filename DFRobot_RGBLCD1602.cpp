...
void DFRobot_RGBLCD1602::init()
{
  ...
  conf.master.clk_speed = 100000;  // Standard 100kHz speed for I2C
...
void DFRobot_RGBLCD1602::print(const char* s) {
	if (!s) return;
	while (*s) {
...
{
    (void)i2c_master_write_to_device(I2C_PORT,_lcdAddr, data, len, pdMS_TO_TICKS(50));
    vTaskDelay(pdMS_TO_TICKS(100));
}

void DFRobot_RGBLCD1602::setReg(uint8_t addr, uint8_t value)
{ 
  uint8_t data[2] = {addr, value}; 
  (void)i2c_master_write_to_device(I2C_PORT, _RGBAddr, data, 2, pdMS_TO_TICKS(50));
}
